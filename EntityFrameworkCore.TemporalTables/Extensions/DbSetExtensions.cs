...
namespace EntityFrameworkCore.TemporalTables.Extensions
{
    public static class DbSetExtensions
    {
        ...
        /// <remarks>
        ///     The same entity might be returned more than once if it was modified
        ///     during that time frame.
        /// </remarks>
        public static IQueryable<TEntity> Between<TEntity>(this DbSet<TEntity> dbSet, DateTimeOffset startDate,
            ...
            where TEntity : class
        {
            ...
            var sql = FormattableString.Invariant(
                $"SELECT * FROM [{GetTableName(dbSet)}] FOR SYSTEM_TIME BETWEEN {{0}} AND {{1}}");
            ...
            if (!TemporalEntitiesCache.IsEntityConfigurationTemporal(entityType))
                throw new ArgumentException(
                    FormattableString.Invariant(
                        $"The entity '{entityType.DisplayName()}' is not using temporal tables."), nameof(dbSet));
        }
